...
const syncArmyWithStore = () => {
  army.value = armyStore.loadDedicatedTransportsForArmy(props.armyIndex);
  sortArmyByUnitName();
...
const addUnitToArmy = async (unit) => {
  try {
    ...
    armyStore.addDedicatedTransportToArmy(props.armyIndex, unitWithId);
    syncArmyWithStore();
  } catch (error) {
...
const removeUnitFromArmy = (id) => {
  armyStore.removeDedicatedTransportFromArmy(props.armyIndex, id);
  syncArmyWithStore();
};
...
const loadUnits = () => {
  fetch(props.url)
    ...
    .then(data => {
      units.value = data.dedicatedTransports;
    })
    .catch(error => {
